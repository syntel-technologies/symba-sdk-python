...
Promote `dev` to protected `main` through a reviewed merge PR. Do not force-push published history. Release Please proposes versions and release notes; review the generated release PR before it can merge. The owner may review and merge their own PR after the required checks pass. GitHub does not allow self-approval, so there is no separate approval requirement; direct pushes and protection bypasses remain blocked.
