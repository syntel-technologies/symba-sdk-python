...
3. The `main` rules require a PR, resolved review conversations, passing checks on an up-to-date branch, and no blocking CodeQL findings. Required approvals are **zero**: the owner can review and merge their own PR once checks pass; GitHub does not permit approving your own PR. No approval from `res1988` is required. Repository administrators have no bypass. `Required checks` aggregates the mandatory CI jobs so a skipped/failed dependency cannot look green. `PR title` validates the review title.
...
`Release` attaches the validated wheel and source distribution to GitHub. PyPI publishing remains **disabled** until the project owner creates a PyPI account and configures a pending Trusted Publisher for `syntel-symba`. There is no stored PyPI API token.
